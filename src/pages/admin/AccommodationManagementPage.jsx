...
import { BedDouble, CalendarDays, CheckCircle2, Download, Eye, IndianRupee, Mail, Plus, RefreshCw, Search, Settings2, Trash2, UserRound, UsersRound, X } from 'lucide-react';
...
const occupancyLabel = (booking = {}) => {
  ...
  if (booking.occupancyLabel) return booking.occupancyLabel;
...
const freshForm = (hotel = {}) => ({
  accommodationId: hotel._id || '', occupancyType: 'SINGLE', sharingWith: 'OTHER', roommateName: '', ...initialStayDates(hotel),
  checkInTime: hotel.checkInTime || '14:00', checkOutTime: hotel.checkOutTime || '12:00', amountCollected: '', amountAdjustmentNote: '',
  paymentMethod: 'UPI', paymentReference: '', paymentDate: todayInIndia(), adminNotes: '', confirmPaymentReceived: false,
});
...
const AccommodationManagementPage = () => {
  ...
  useEffect(() => {
    ...
    let current = true;
    adminAPI.getAccommodationQuote({ accommodationId: form.accommodationId, occupancyType: form.occupancyType, sharingWith: form.sharingWith, checkInDate: form.checkInDate, checkOutDate: form.checkOutDate })
      .then(({ data }) => {
        if (!current) return;
        setQuote(data.quote);
  ...
  }, [form.accommodationId, form.occupancyType, form.sharingWith, form.checkInDate, form.checkOutDate]);
  ...
  const change = ({ target }) => setForm((current) => {
    ...
    if (target.name === 'occupancyType' && value === 'SINGLE') {
      next.sharingWith = 'OTHER';
      next.roommateName = '';
    }
  ...
  const deleteBooking = async (booking) => {
    const confirmed = window.confirm(`Delete accommodation booking ${booking.bookingNumber} for ${booking.userId?.name || 'this delegate'}? This also removes its accommodation payment record and cannot be undone.`);
    if (!confirmed) return;
    setNotice(null);
    try {
      const { data } = await adminAPI.deleteAccommodationBooking(booking._id);
      setBookings((items) => items.filter((item) => item._id !== booking._id));
      if (invoicePreview?.booking?._id === booking._id) closeInvoicePreview();
      setNotice({ error: false, text: data.message || `${booking.bookingNumber} deleted successfully.` });
    } catch (error) {
      setNotice({ error: true, text: error.response?.data?.message || 'Accommodation booking could not be deleted.' });
    }
  };
  ...
  return <div className="flex min-h-screen bg-slate-50"><Sidebar /><main className="min-w-0 flex-1 p-4 sm:p-6"><div className="mx-auto max-w-7xl space-y-5">
    ...
    {tab === 'create' && <div className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]"><div className="space-y-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><h2 className="font-bold text-slate-950">2. Hotel and stay</h2><div className="mt-4 grid gap-4 sm:grid-cols-2">
        ...
        <label className="text-sm font-medium">Occupancy<select name="occupancyType" value={form.occupancyType} onChange={change} className={inputClass}><option value="SINGLE">Single occupancy</option><option value="SHARING">Sharing</option></select></label>
        {form.occupancyType === 'SHARING' && <label className="text-sm font-medium">Sharing with<select name="sharingWith" value={form.sharingWith} onChange={change} className={inputClass}><option value="FAMILY">Family</option><option value="FACULTY">Other faculty</option><option value="OTHER">Other delegate</option></select></label>}
      ...
      <div className="mt-4 space-y-3 rounded-xl border bg-slate-50 p-4 text-sm"><label className="flex gap-3"><input type="checkbox" name="confirmPaymentReceived" checked={form.confirmPaymentReceived} onChange={change} />I confirm payment was received.</label><p className="text-xs text-slate-500">No email is sent yet. The invoice opens for review after the booking is recorded.</p></div>
      <button type="button" onClick={createBooking} disabled={busy || !selectedDelegate || !selectedHotel || !quote || !form.confirmPaymentReceived} className="mt-5 w-full rounded-xl bg-[#005aa9] px-4 py-3 text-sm font-bold text-white disabled:opacity-50">{busy ? 'Recording…' : 'Record and preview invoice'}</button>
    </section></div>}

    {tab === 'bookings' && <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold">Accommodation bookings</h2><p className="text-xs text-slate-500">Internal allocations linked to delegate profiles.</p></div><div className="relative"><Search className="absolute left-3 top-2.5 h-4 w-4 text-slate-400" /><input value={bookingSearch} onChange={(event) => setBookingSearch(event.target.value)} placeholder="Delegate, hotel or booking" className="rounded-xl border py-2 pl-9 pr-3 text-sm" /></div></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[1120px] text-left text-sm"><thead><tr className="border-b text-xs uppercase text-slate-500"><th className="p-3">Delegate</th><th className="p-3">Hotel</th><th className="p-3">Stay</th><th className="p-3">Occupancy</th><th className="p-3">Collected</th><th className="p-3">Payment</th><th className="p-3">Invoice & email</th></tr></thead><tbody className="divide-y">{filteredBookings.map((item) => <tr key={item._id}><td className="p-3"><p className="font-bold">{item.userId?.name}</p><p className="text-xs text-slate-500">{item.bookingNumber} · {item.userId?.phone}</p></td><td className="p-3"><p className="font-medium">{item.accommodationId?.name || '—'}</p><p className="text-xs text-slate-500">{item.accommodationId?.location}</p></td><td className="p-3"><p>{dateLabel(item.checkInDate)} – {dateLabel(item.checkOutDate)}</p><p className="text-xs text-slate-500">{item.numberOfNights} nights · {item.checkInTime} / {item.checkOutTime}</p></td><td className="p-3"><span className="rounded-full bg-sky-50 px-2 py-1 text-xs font-bold text-sky-700">{occupancyLabel(item)}</span>{item.roommateName && <p className="mt-1 text-xs text-slate-500">with {item.roommateName}</p>}</td><td className="p-3 font-bold">{money(item.amountCollected ?? item.totalAmount)}</td><td className="p-3"><p>{item.paymentMethod?.replaceAll('_', ' ') || '—'}</p><p className="max-w-44 truncate text-xs text-slate-500">{item.paymentReference || 'No reference'}</p></td><td className="p-3"><div className="flex flex-wrap items-center gap-2"><button type="button" onClick={() => previewBooking(item)} className="flex items-center gap-1 rounded-lg border border-slate-300 px-2 py-1 text-xs font-bold text-slate-700"><Eye className="h-3 w-3" />Preview</button>{item.paymentEmailSentAt ? <span className="text-xs font-bold text-emerald-700">Sent {dateLabel(item.paymentEmailSentAt)}</span> : <button type="button" onClick={() => emailBooking(item)} className="flex items-center gap-1 rounded-lg border border-[#005aa9]/30 px-2 py-1 text-xs font-bold text-[#005aa9]"><Mail className="h-3 w-3" />Send email</button>}<button type="button" onClick={() => deleteBooking(item)} className="flex items-center gap-1 rounded-lg border border-red-200 px-2 py-1 text-xs font-bold text-red-700 hover:bg-red-50"><Trash2 className="h-3 w-3" />Delete</button></div></td></tr>)}</tbody></table>{!filteredBookings.length && <p className="py-10 text-center text-sm text-slate-500">No bookings found.</p>}</div></section>}
