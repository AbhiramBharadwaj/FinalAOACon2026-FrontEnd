...
const AttendanceManagementPage = () => {
  ...
  const fetchAttendances = async () => {
    try {
      const response = await fetch('http://13.60.209.145:5050/api/attendance', {
  ...
  const downloadQR = async (attendanceId) => {
    ...
    link.href = `http://13.60.209.145:5050/api/attendance/qr-download/${attendanceId}`;
  ...
  const exportList = async (type) => {
    ...
    try {
      const endpoint = type === 'attended' 
        ? 'http://13.60.209.145:5050/api/attendance/export-attended' 
        : 'http://13.60.209.145:5050/api/attendance/export-not-attended';
