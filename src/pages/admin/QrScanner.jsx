...
const QrScanner = () => {
  ...
  const handleScan = async (code) => {
    ...
    try {
      ...
      const checkRes = await fetch('http://13.60.209.145:5050/api/attendance/scan/check', {
        method: 'POST',
        headers: {
      ...
      const markRes = await fetch('http://13.60.209.145:5050/api/attendance/scan/mark', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('adminToken') || localStorage.getItem('token')}`,
  ...
  useEffect(() => {
    const fetchRegs = async () => {
      try {
        const res = await fetch('http://13.60.209.145:5050/api/admin/registrations', {
          headers: {
            Authorization: `Bearer ${localStorage.getItem('adminToken') || localStorage.getItem('token')}`,
          },
        });
